...
# لیست تگ‌های جدید
DEFAULT_TAGS = "موز,فال,کباب"

# توابع کمکی برای تعامل با API
...
def load_clusters_from_file():
    """بارگذاری دسته‌بندی از فایل cluster.json"""
    try:
...
def save_clusters_to_file(clustered_tags):
    """ذخیره دسته‌بندی در فایل cluster.json"""
    try:
        with open("cluster.json", "w", encoding="utf-8") as f:
            json.dump(clustered_tags, f, ensure_ascii=False, indent=2)
        return True
    except Exception as e:
...
with tab2:
        ...
        if st.button("🔄 به‌روزرسانی دسته‌بندی", use_container_width=True, key="update_clusters"):
            if st.session_state.api_key and st.session_state.all_tags.strip():
                with st.spinner("در حال دسته‌بندی تگ‌ها..."):
                    clustered_data = cluster_tags_from_api(st.session_state.all_tags, st.session_state.api_key)
                    st.session_state.clustered_tags = clustered_data
                    # ذخیره در فایل
                    if clustered_data:
                        save_clusters_to_file(clustered_data)
    ...
    with col2:
        if st.button("🗑️ حذف فایل دسته‌بندی", use_container_width=True, key="delete_clusters"):
            try:
                if os.path.exists("cluster.json"):
                    os.remove("cluster.json")
                    st.session_state.clustered_tags = []
                    st.success("✅ فایل دسته‌بندی حذف شد!")
                else:
                    st.info("ℹ️ فایل دسته‌بندی وجود ندارد.")
                st.rerun()
            except Exception as e:
    ...
    # نمایش JSON دسته‌بندی
    if st.session_state.show_json and st.session_state.clustered_tags:
        st.markdown('<div class="custom-card">', unsafe_allow_html=True)
        st.subheader("📄 JSON دسته‌بندی تگ‌ها")
        
        json_str = json.dumps(st.session_state.clustered_tags, ensure_ascii=False, indent=2)
        st.markdown('<div class="json-display">', unsafe_allow_html=True)
        st.code(json_str, language="json")
        st.markdown('</div>', unsafe_allow_html=True)
        
        # دکمه کپی JSON
        if st.button("📋 کپی JSON", key="copy_json"):
            st.success("✅ JSON کپی شد!")
    ...
    # ابتدا چک کن فایل وجود داره
    if not st.session_state.clustered_tags:
        file_clusters = load_clusters_from_file()
        if file_clusters:
            st.session_state.clustered_tags = file_clusters
            st.info("ℹ️ دسته‌بندی‌ها از فایل cluster.json بارگذاری شدند.")
